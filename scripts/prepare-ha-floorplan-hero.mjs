...
function extractTargets(zip) {
  ...
  if (extracted.size !== TARGETS.size) {
    ...
    throw new Error(`Hero archive is missing expected files: ${missing.join(", ")}`);
  }
  return extracted;
}
...
async function main() {
  ...
  if (actualHash !== EXPECTED_SHA256) {
    throw new Error(`HAFloorplan hero archive integrity check failed. Expected ${EXPECTED_SHA256}, received ${actualHash}.`);
  }
