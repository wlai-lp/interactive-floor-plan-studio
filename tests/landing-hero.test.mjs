...
test("landing page uses shared shell, static hero, workflow, and trust strip", () => {
  ...
  assert.match(hero, /VISUAL EDITOR/);
  assert.match(hero, /HOME ASSISTANT/);
...
test("landing page has no demo video or animation wiring", () => {
  assert.doesNotMatch(hero, /Watch 30-second demo/);
  assert.doesNotMatch(hero, /HAFloorPlanHero|HAFloorPlanPromo|next\/dynamic|import\(/);
  assert.doesNotMatch(hero, /role="dialog"|aria-modal/);
